...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // === SATPAM KEAMANAN (Route Protection) ===
  useEffect(() => {
    const checkAccess = () => {
      const storedUser = localStorage.getItem("nextTeaUser");
      if (!storedUser) {
        // Jika tidak ada data login, tendang ke halaman login
  ...
  // Tampilkan layar loading keren sebentar saat sedang mengecek akses
  ...
  // Master Menu
  const allMenuItems = [
    { name: "Ringkasan Bisnis", icon: LayoutDashboard, path: "/admin/dashboard" },
    { name: "Kasir & Transaksi", icon: ShoppingCart, path: "/admin/transactions" },
    { name: "Stok & Bahan Baku", icon: Package, path: "/admin/inventory" },
    { name: "Laporan Keuangan", icon: BarChart3, path: "/admin/reports" },
    { name: "Kelola Pengguna", icon: Users, path: "/admin/users" },
    { name: "Pelanggan & Member", icon: Award, path: "/admin/customers" }, 
  ];

  // LOGIKA PEMBATASAN AKSES (RBAC)
  const menuItems = allMenuItems.filter((item) => {
    if (user.role === "Kasir") {
      return item.name === "Kasir & Transaksi" || item.name === "Laporan Keuangan";
    }
    if (user.role === "Staff Inventory") {
      return item.name === "Stok & Bahan Baku";
    }
    return true; 
  });
      ...
      <aside className="w-64 bg-[#4318FF] text-white flex flex-col z-20 relative shadow-2xl md:rounded-r-[2.5rem] flex-shrink-0">
        ...
        <div className="p-8 flex items-center gap-3 relative z-10">
          ...
          <div>
            ...
            <p className="text-[9px] text-indigo-200 font-bold tracking-widest uppercase mt-0.5">POS System</p>
          </div>
        </div>
            ...
            return (
              <Link key={item.name} href={item.path}>
                <div className={`flex items-center gap-4 px-4 py-3.5 rounded-2xl font-bold transition-all duration-200 ${
                  isActive 
                    ? "bg-white text-[#4318FF] shadow-md scale-[1.02]" 
        ...
        <div className="p-6 relative z-10 mt-auto">
          <button 
            onClick={logout}
            className="flex items-center gap-3 w-full px-4 py-3.5 rounded-2xl bg-white/10 hover:bg-rose-500 text-white transition-all border border-transparent shadow-sm cursor-pointer group"
          >
